...
#define arena_push_array(arena, Type, count) (Type *)arena_push_size(arena, count * sizeof(Type))

/// Pops a struct from the arena stack; this is effective only if the struct
/// matches with the last allocation
#define arena_pop_struct(arena, Type, ptr) arena_pop_size(arena, sizeof(Type), ptr)

/// Pops an array of structs from the arena stack; this is effective only if the
/// array matches with the last allocation
#define arena_pop_array(arena, Type, count, ptr) arena_pop_size(arena, count * sizeof(Type), ptr)
